{serendipity_hookPlugin hook="entries_header" addData="$entry_id"}
  ...
  {assign var="prentries" value="No"}
 


  {if ($is_single_entry =! '1' || $view=='archives')} {assign var="prentries" value="1"}   {/if}
  {if ($is_single_entry =! '1' || $view=='categories')} {assign var="prentries" value="1"}   {/if}
    {if ($is_single_entry =! '1' || $view=='plugin')} {assign var="prentries" value="1"}   {/if}
  {if $view=='start' }  {assign var="prentries" value="1"}   {/if}
  {if $view=='search' }  {assign var="prentries" value="1"}   {/if}




 {if $prentries=='1' } 
   {include file='entries_pr.tpl'}
...
{foreach from=$entries item="dategroup"}
    ...
    <article id="post_{$entry.id}" class="post{if !$is_single_entry and not $entry.is_extended and not $is_preview}-preview{/if} serendipity_entry{if $dategroup.is_sticky} sticky {/if} white-sidebar t10pixel" role="article">
        ...
        <section id="entry"> 
            <div class="content serendipity_entry_body clearfix">
              ...
              {if $entry.is_entry_owner and not $is_preview}
				<a href="{$entry.link_edit}"  title="{$CONST.EDIT_ENTRY}"><button class="btn btn-sm btn-default"><i class="fa fa-lg fa-edit"></i><span class="sr-only">{$CONST.EDIT_ENTRY}</span></button></a>
			{/if}


				{if $entry.properties.ep_smallIMG != ''}
					<div class="row">
						<div class="col-sm-5">
							<a href="{$entry.link}"><img class="img-thumbnail img-responsive b10pixel" src="{$entry.properties.ep_smallIMG}" alt="{$entry.properties.ep_smallIMG} image" /></a>
						</div>
						<div class="col-sm-7 entrybody">
						<small> <time datetime="{$entry.timestamp|@serendipity_html5time}">{$entry.timestamp|@formatTime:$template_option.date_format}</small> </time>	{$entry.body}
						</div>
					</div>	
					{else}
						<div class="entrybody">
            ...
            {if $entry.categories or $entry.add_footer}
                <footer class="entry-footer">
                    ...
                    {else}
                        {$entry.freetag} 
                    {/if}   
  {include file='include_related_entries2.tpl'}  					
                    {$entry.add_footer} 
                </footer>
            {/if}
        </section>
