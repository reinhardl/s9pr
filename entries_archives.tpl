...
<article class="archive white-sidebar t10pixel">
                    ...
                    {foreach from=$archive.months item="month"}
                        <div class="col-md-3 col-sm-4 col-xs-6">
                            {if $template_option.entrybody_detailed_only==true}
                                ...
                                {else}                                            
                                    <button type="button" class="btn btn-default disabled archive-button minbr"><span class="text-muted">{$month.date|@formatTime:"%b"}&nbsp;</span></button>  
                                {/if}
                            {else}
                                <div class="btn-group archive-button">
                                    {if $month.entry_count}
                                        <button type="button" class="btn btn-default dropdown-toggle archive-button minbr" data-toggle="dropdown" aria-expanded="false">{$month.date|@formatTime:"%b"}&nbsp;<span class="badge">{$month.entry_count}</span>&nbsp;&nbsp;<span class="caret"></span></button>
                                    ...
                                    {else}
                                        <button type="button" class="btn btn-default disabled dropdown-toggle archive-button minbr" data-toggle="dropdown" aria-expanded="false">{$month.date|@formatTime:"%b"}</button>
                                    {/if}
                                </div>                                
                            {/if}
                        </div>
